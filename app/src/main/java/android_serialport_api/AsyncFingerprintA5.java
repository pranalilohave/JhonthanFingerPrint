...
import in.co.ashclan.fgtit.utils.DataUtils;

public class AsyncFingerprintA5 extends Handler {
	...
	protected AsyncFingerprintA5(Looper looper) {
	...
	protected class WorkerHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case FP_GetImage:
				...
				AsyncFingerprintA5.this.obtainMessage(FP_GetImage, valueGetImage,
						-1).sendToTarget();
				break;
			case FP_UpImage:
				byte[] imageData = PSUpImage();
				AsyncFingerprintA5.this.obtainMessage(FP_UpImage, imageData)
						.sendToTarget();
				break;
			case FP_GenChar:
				int valueGenChar = PSGenChar(msg.arg1);
				AsyncFingerprintA5.this.obtainMessage(FP_GenChar, valueGenChar,
						msg.arg1).sendToTarget();
				break;
			case FP_RegModel:
				int valueRegModel = PSRegModel();
				AsyncFingerprintA5.this.obtainMessage(FP_RegModel, valueRegModel,
						-1).sendToTarget();
				break;
			case FP_UpChar:
				byte[] charData = PSUpChar();
				AsyncFingerprintA5.this.obtainMessage(FP_UpChar, charData)
						.sendToTarget();
				break;
			case FP_DownChar:
				int valueDownChar = PSDownChar((byte[]) msg.obj);
				AsyncFingerprintA5.this.obtainMessage(FP_DownChar, valueDownChar,
						-1).sendToTarget();
				break;
			case FP_Match:
				boolean valueMatch = PSMatch();
				AsyncFingerprintA5.this.obtainMessage(FP_Match,
						Boolean.valueOf(valueMatch)).sendToTarget();
				break;
			case FP_StoreChar:
				int valueStoreChar = PSStoreChar(msg.arg1, msg.arg2);
				AsyncFingerprintA5.this.obtainMessage(FP_StoreChar,
						valueStoreChar, -1).sendToTarget();
				break;
			case FP_LoadChar:
				int valueLoadChar = PSLoadChar(msg.arg1, msg.arg2);
				AsyncFingerprintA5.this.obtainMessage(FP_LoadChar, valueLoadChar,
						-1).sendToTarget();
				break;
			case FP_Search:
				byte[] result = PSSearch(msg.arg1, msg.arg2, (Integer) msg.obj);
				AsyncFingerprintA5.this.obtainMessage(FP_Search, result)
						.sendToTarget();
				break;
			case FP_DeleteChar:
				int valueDeleteChar = PSDeleteChar((short) msg.arg1,
						(short) msg.arg2);
				AsyncFingerprintA5.this.obtainMessage(FP_DeleteChar,
						valueDeleteChar, -1).sendToTarget();
				break;
			case FP_Empty:
				int valueEmpty = PSEmpty();
				AsyncFingerprintA5.this.obtainMessage(FP_Empty, valueEmpty, -1)
						.sendToTarget();
				break;
			case FP_Enroll:
				byte[] valueEnroll = PSEnroll();
				AsyncFingerprintA5.this.obtainMessage(FP_Enroll, valueEnroll)
						.sendToTarget();
				break;
			case FP_Identify:
				byte[] valueIdentify = PSIdentify();
				AsyncFingerprintA5.this.obtainMessage(FP_Identify, valueIdentify)
						.sendToTarget();
				break;
			case FP_GetImageEX:
				int valueGetImageEx = PSGetImageEx();
				AsyncFingerprintA5.this.obtainMessage(FP_GetImageEX, valueGetImageEx,
						-1).sendToTarget();
				break;
			case FP_UpImageEX:
				byte[] imageDataEx = PSUpImageEx();
				AsyncFingerprintA5.this.obtainMessage(FP_UpImageEX, imageDataEx)
						.sendToTarget();
				break;
			case FP_GenCharEX:
				int valueGenCharEx = PSGenCharEx(msg.arg1);
				AsyncFingerprintA5.this.obtainMessage(FP_GenCharEX, valueGenCharEx,
						msg.arg1).sendToTarget();
				break;
			default:
				break;
	...
	private synchronized int PSGetImage() {
		...
		sendCommand(command);		
		int length = SerialPortManagerA5.getInstance().read(buffer,12,300);
	...
	private synchronized byte PSGenChar(int bufferId) {
		byte[] command = { (byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff,
				...
				(byte) (0x7 + bufferId) };
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer,12,500);
	...
	private synchronized byte PSRegModel() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, 12,100);
	...
	private synchronized byte[] PSSearch(int bufferId, int startPageId,
		...
		byte[] command = { (byte) 0xef, (byte) 0x01, (byte) 0xff, (byte) 0xff,
				...
				(byte) checkSumArray[0], (byte) checkSumArray[1] };
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, 16,100);
	...
	private synchronized boolean PSMatch() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, 14,100);
	...
	private synchronized byte[] PSEnroll() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, 14,600);
	...
	private synchronized byte[] PSIdentify() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, 16,600);
	...
	private synchronized byte[] PSUpChar() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, UP_CHAR_RESPONSE_SIZE,100);
	...
	private synchronized byte[] PSUpImage() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, UP_IMAGE_RESPONSE_SIZE,100);
	...
	private synchronized int PSGetImageEx() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, 12,250);
	...
	private synchronized byte[] PSUpImageEx() {
		...
		sendCommand(command);
		int length = SerialPortManagerA5.getInstance().read(buffer, UP_IMAGEEX_RESPONSE_SIZE,100);
	...
	private byte[] toBmpByte(int width, int height, byte[] data) {
		...
		try {
			...
			int bfSize = 54 + 1024 + width * height;// bmp??????��??2??5????
...
//		Log.i("whw", "sendCommand hex=" + DataUtils.toHexString(command));
		try {
			SerialPortManagerA5.getInstance().write(command);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
